...
if ($_GET['token']) {
    ...
    $query = "SELECT * FROM TRAN_USERS WHERE tokenR = :tokenR";
    $conditions = [
        [":tokenR", $token]
    ];
    $DoesTokenExist = $db->query($query, $conditions);

    if (!empty($DoesTokenExist)) {
        //Token trouvé, on affiche le formulaire :
        ?>
        <!DOCTYPE html>
        <html lang="fr">
                    ...
                    <div class="col-lg-4 col-12 d-flex flex-column justify-content-center p-5 col-login">
                        <form action="./includes/updatePassword.php" method="post">
                            ...
                            <div class="row">
                                ...
                                <?php echo"<input type='hidden' name='token' id='token' value='$token'>";?>
                                <div class="col-2 col-md-1 col-lg-2 col-xxl-1 d-flex align-items-center justify-content-center p-0 h-100 m-0 pe-3" id="col-eye">
